...
function convertSupabaseToProfileData(user: any): ProfileData {
  return {
    ...
    bioTagline: user.bio_tagline || user.bio?.substring(0, 100) || '',
    ...
    faith: 'strong',
    diet: 'halal_only',
    alcohol: user.alcohol || 'never',
    smoking: user.smoking || 'never',
    psychedelics: user.psychedelics || 'never',
    halalFood: user.halal_food || 'always',
    ...
    photos: user.profile_photos || [],
    walletAddress: user.principal || user.solana_address,
...
function convertProfileDataToSupabase(walletAddress: string, profileData: Partial<ProfileData>): any {
  return {
    ...
    location: profileData.location || profileData.currentLocation || '',
    latitude: profileData.latitude,
    longitude: profileData.longitude,
    city: (profileData as any).city || profileData.currentLocation?.split(',')[0]?.trim(),
    state: (profileData as any).state || undefined,
    country: (profileData as any).country || undefined,
    bio_tagline: profileData.bioTagline,
    ...
    halal_food: profileData.halalFood,
    bio: profileData.bio,
