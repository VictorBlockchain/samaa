...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: linkData, error: linkError } = await supabaseAdmin.auth.admin.generateLink({
      ...
      email,
      options: {
        redirectTo: `${process.env.NEXT_PUBLIC_APP_URL || process.env.NEXT_PUBLIC_SITE_URL || "https://samaa.app"}/auth/verify-email`,
      },
    })

    // Send verification email via Mailgun
    console.log("[Signup] Sending verification email...")
    const verificationLink = `${process.env.NEXT_PUBLIC_APP_URL || process.env.NEXT_PUBLIC_SITE_URL || "https://samaa.app"}/auth/verify-email?token=${authData.user.id}&email=${encodeURIComponent(email)}`
